...
def test_deserialize_with_type_metadata_basic(monkeypatch):
    ...
    assert _deserialize_with_type_metadata(
        {"__datason_type__": "datetime", "__datason_value__": dt_str}
    ) == datetime.fromisoformat(dt_str)
    assert _deserialize_with_type_metadata(
        {"__datason_type__": "uuid.UUID", "__datason_value__": uuid_str}
    ) == uuid.UUID(uuid_str)
    assert (
        _deserialize_with_type_metadata({"__datason_type__": "complex", "__datason_value__": {"real": 1, "imag": 2}})
        == 1 + 2j
    )
    assert _deserialize_with_type_metadata(
        {"__datason_type__": "decimal.Decimal", "__datason_value__": "3.14"}
    ) == Decimal("3.14")
    assert _deserialize_with_type_metadata({"__datason_type__": "pathlib.Path", "__datason_value__": "/tmp"}) == Path(
        "/tmp"
    )
    ...
    df_meta = {
        ...
        "__datason_value__": {"index": [0, 1], "columns": ["a"], "data": [[1], [2]]},
    }
...
def test_deserialize_with_numpy_and_torch(monkeypatch):
    ...
    ndarray_meta = {
        ...
        "__datason_value__": {"data": [1, 2, 3, 4], "dtype": "float32", "shape": [2, 2]},
    }
    ...
    skl_meta = {
        ...
        "__datason_value__": {"_class": "sklearn.linear_model.LinearRegression", "_params": {"a": 1}},
    }
